...
def main() -> int:
    ...
    with TdxClient(timeout=6, heartbeat_interval=None) as client:
        response = client.get_trades(args.code, args.date, count=args.count)
        print(f"[OK] trades code={response.full_code} date={response.trading_date} count={response.count}")
    return 0
